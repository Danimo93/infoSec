...
if (!isset( $_COOKIE["flowershop_cart"] )) {
	echo "<p>Your cart is empty, you cannot remove any items";
}
else{
	// get old cart value
	...
	$result=db_query("delete from flowercart where cartid=$cartid and flowerid=".$_GET["id"]);

	// if no more flowers in cart, remove the quantity
	$result=db_query("select * from flowercart where cartid=$cartid");
	if (num_rows($result)==0){
		$result=db_query("update cart set flowerquantity=1 where uid=$cartid");
	}
?>

<p class="content">Flowers removed from cart.   <a href="checkout.php">Click here</a> to go back to checkout
